...
* if the `fetch` function returns a `Flowable` that delivers more than the requested number of items then a `MissingBackpressureException` is emitted and the overal stream terminated.
