...
public final class FlowableOnBackpressureBufferToFile<T> extends Flowable<T> {
	...
	private static final class BufferToFileSubscriberFlowable<T> extends BufferToFileSubscriber<T>
		...
		BufferToFileSubscriberFlowable(Subscriber<? super T> child, PagedQueue queue, Serializer<T> serializer,
		        Worker worker) {
	...
	@SuppressWarnings({ "serial" })
	private static abstract class BufferToFileSubscriber<T> extends AtomicInteger implements Runnable {
